...
into nine classes by what the closing is doing rather than by what it says:
	...
	exttt{ack}. Appendix~ef{app:classes} gives the full table. The grouping
matters because the loop of Section~ef{sec:loop} tunes classes rather than
...
Two implementation details proved necessary rather than cosmetic. First, all
...
tune, because tuning needs labels and labels need a reader. We showed that both
labels this system needs are already lying in the transcript: the user's own
push before the gate exists, and the agent's own behaviour after the gate
intervenes. Neither costs an annotation. Together they let the gate report on
itself, and the first thing it reported was a pattern of its author's that
condemned nothing but reports.
