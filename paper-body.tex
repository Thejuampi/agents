...
The headline is uncomfortable and we lead with it. The failure happens on about
one closing in ten. The gate fires on three in four and is right about one in
eight, which is the rate at which the failure occurs at all, and no lane
configuration we measured separates from that rate at 95\%. The same
transcripts then say the interventions are mostly worth making: 35 of 41
graded blocks bought work. Both statements are measured, neither cancels the
other, and the second is the one that keeps the gate switched on.
...
reading this corpus and reported on it. Section~\ref{sec:results} splits the
...
it. The cheaper lanes should take everything they can prove, and the marginal
cut says how much that is worth here: on the closings only the patterns catch,
precision is 0.145; on the closings only the judge catches, it is 0.106 on a
base rate of 0.104. The model earns its place by covering three times the
volume, and it earns none of it by being more discriminating than a regular
expression.
...
messages are worth a model at all. The judge's own confidence was informative
and unsafe to threshold, because dropping a low-confidence STOP loses real
pushes. It became usable when the action space grew a third option: a STOP
under the floor is put as the question instead of as an accusation, and the
turn is neither blocked nor released. A weak signal that cannot pick between
two actions can still pick a third.
