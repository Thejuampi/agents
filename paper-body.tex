...
produced three rules. That path is manual and it is only open on ungated
transcripts, where the label is not the gate's own output. Under the gate the
push label goes sparse, which is the mechanism that makes the loop harder to
close the better the gate gets.
